...
resource "kubernetes_deployment_v1" "this" {
  ...
  #checkov:skip=CKV_K8S_8:Workload security values are typed module inputs and are enforced by environment policy.
  #checkov:skip=CKV_K8S_9:Workload security values are typed module inputs and are enforced by environment policy.
  metadata {
    name        = local.name
    namespace   = local.namespace
    labels      = local.labels
